...
TrajectoryGeneratorROS::TrajectoryGeneratorROS(
    ...
    RCLCPP_INFO(this->get_logger(),
                "TrajectoryGeneratorROS initialized with Debug Mode = [%s]",
...
void TrajectoryGeneratorROS::registerTrajectoryGenerator(
...
nav2_util::CallbackReturn
TrajectoryGeneratorROS::on_activate(const rclcpp_lifecycle::State &state) {
    ...
    this->action_server_ = rclcpp_action::create_server<TrajectoryGeneration>(
        ...
        std::bind(&TrajectoryGeneratorROS::handle_goal, this,
                  std::placeholders::_1, std::placeholders::_2),
        std::bind(&TrajectoryGeneratorROS::handle_cancel, this,
                  std::placeholders::_1),
        std::bind(&TrajectoryGeneratorROS::handle_accepted, this,
                  std::placeholders::_1));
    for (size_t i = 0; i < this->trajectory_generators.size(); i++) {
...
nav2_util::CallbackReturn
TrajectoryGeneratorROS::on_deactivate(const rclcpp_lifecycle::State &state) {
...
nav2_util::CallbackReturn
TrajectoryGeneratorROS::on_cleanup(const rclcpp_lifecycle::State &state) {
...
nav2_util::CallbackReturn
TrajectoryGeneratorROS::on_shutdown(const rclcpp_lifecycle::State &state) {
    RCLCPP_DEBUG(this->get_logger(), "on_shutdown");
    return nav2_util::CallbackReturn::SUCCESS;
}

/* Action server */
rclcpp_action::GoalResponse TrajectoryGeneratorROS::handle_goal(
    const rclcpp_action::GoalUUID &uuid,
    std::shared_ptr<const TrajectoryGeneration::Goal> goal) {
    RCLCPP_DEBUG(get_logger(), "received goal - UUID=[%s]",
