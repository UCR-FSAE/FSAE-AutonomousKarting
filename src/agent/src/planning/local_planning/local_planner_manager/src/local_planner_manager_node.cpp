...
LocalPlannerManagerNode::LocalPlannerManagerNode()
    : LifecycleNode("manager", "local_planner", true) {
    ...
    this->controllerServerRoute =
        this->get_parameter("controller_route").as_string();

    RCLCPP_INFO(this->get_logger(),
                "LocalPlannerManagerNode initialized with Debug Mode = [%s]",
                this->get_parameter("debug").as_bool() ? "YES" : "NO");
    RCLCPP_INFO(this->get_logger(), "controller route: [%s]",
                this->controllerServerRoute.c_str());
}
LocalPlannerManagerNode ::~LocalPlannerManagerNode() {}
...
LocalPlannerManagerNode::on_configure(const rclcpp_lifecycle::State &state) {
    RCLCPP_DEBUG(this->get_logger(), "on_configure");

    this->next_waypoint_sub_ =
        this->create_subscription<geometry_msgs::msg::Pose>(
            "/next_waypoint", rclcpp::SystemDefaultsQoS(),
            std::bind(&LocalPlannerManagerNode::onLatestWaypointReceived, this,
                      std::placeholders::_1));
    ...
    trajectory_generator_thread_ =
        std::make_unique<nav2_util::NodeThread>(trajectory_generator_node_);
    this->register_generators();
    trajectory_generator_node_->configure();

    trajectory_picker_node_ =
        std::make_shared<local_planning::TrajectoryPickerROS>(
            "picker", std::string{get_namespace()}, "trajectory",
            this->get_parameter("debug").as_bool());
    trajectory_picker_thread_ =
        std::make_unique<nav2_util::NodeThread>(trajectory_picker_node_);
    trajectory_picker_node_->configure();

    this->possible_trajectory_publisher_ =
        this->create_publisher<planning_interfaces::msg::Trajectory>(
            std::string{get_namespace()} + "/trajectory/picker" +
                "/possible_trajectory",
            10);

    costmap_node_ = rclcpp::Node::make_shared("get_costmap_node");
    costmap_client_ = costmap_node_->create_client<nav2_msgs::srv::GetCostmap>(
        "/local_costmap/get_costmap");

    this->control_action_client_ = rclcpp_action::create_client<ControlAction>(
        this, this->controllerServerRoute);

    return nav2_util::CallbackReturn::SUCCESS;
}
nav2_util::CallbackReturn
LocalPlannerManagerNode::on_activate(const rclcpp_lifecycle::State &state) {
    ...
    execute_timer =
        create_wall_timer(std::chrono::milliseconds(int(loop_rate * 1000)),
    ...
    this->trajectory_generator_client =
        rclcpp_action::create_client<TrajectoryGeneration>(
            this, "trajectory/trajectory_generation");

    this->best_trajectory_subscriber_ = this->create_subscription<
        planning_interfaces::msg::Trajectory>(
        std::string(this->get_namespace()) + "/trajectory/picker" +
            "/best_trajectory",
        10,
        std::bind(
            &LocalPlannerManagerNode::on_best_trajectory_publication_received,
...
nav2_util::CallbackReturn
LocalPlannerManagerNode::on_deactivate(const rclcpp_lifecycle::State &state) {
...
nav2_util::CallbackReturn
LocalPlannerManagerNode::on_cleanup(const rclcpp_lifecycle::State &state) {
...
nav2_util::CallbackReturn
LocalPlannerManagerNode::on_shutdown(const rclcpp_lifecycle::State &state) {
...
void LocalPlannerManagerNode::onLatestWaypointReceived(
    ...
    ps.header = header;
    this->latest_waypoint_ =
        std::make_shared<geometry_msgs::msg::PoseStamped>(ps);
}
void LocalPlannerManagerNode::onLatestOdomReceived(
...
std::shared_ptr<nav2_msgs::msg::Costmap>
LocalPlannerManagerNode::p_GetLatestCostmap() {
...
void LocalPlannerManagerNode::send_trajectory_generator_action(
    ...
    planning_interfaces::action::TrajectoryGeneration_Goal goal_msg =
        TrajectoryGeneration::Goal();
    ...
    send_goal_options.goal_response_callback = std::bind(
        &LocalPlannerManagerNode::trajectory_generator_goal_response_callback,
        this, _1);
    send_goal_options.feedback_callback = std::bind(
        &LocalPlannerManagerNode::trajectory_generator_feedback_callback, this,
        _1, _2);
    send_goal_options.result_callback = std::bind(
        &LocalPlannerManagerNode::trajectory_generator_result_callback, this,
        _1);
...
void LocalPlannerManagerNode::trajectory_generator_goal_response_callback(
    ...
    if (!goal_handle) {
        RCLCPP_ERROR(this->get_logger(), "[trajectory_generator_goal_response_"
...
void LocalPlannerManagerNode::trajectory_generator_feedback_callback(
    ...
    RCLCPP_DEBUG(
        get_logger(),
        "Feedback contains trajectory of length [%d] and raw_score: [%.3f]",
        feedback->trajectory.trajectory.poses.size(),
        feedback->trajectory.score.raw_score);
...
void LocalPlannerManagerNode::trajectory_generator_result_callback(
    ...
    case rclcpp_action::ResultCode::SUCCEEDED:
        RCLCPP_DEBUG(this->get_logger(),
                     "trajectory_generator result received");
        break;
    case rclcpp_action::ResultCode::ABORTED:
        RCLCPP_DEBUG(this->get_logger(),
                     "trajectory_generator goal was aborted");
        break;
    case rclcpp_action::ResultCode::CANCELED:
        RCLCPP_DEBUG(this->get_logger(),
                     "trajectory_generator goal was canceled");
        break;
    default:
        RCLCPP_ERROR(this->get_logger(),
                     "trajectory_generator unknown result code");
        break;
    }
...
void LocalPlannerManagerNode::on_best_trajectory_publication_received(
    ...
    this->control_send_goal(
        std::make_shared<nav_msgs::msg::Path>(msg->trajectory), 10.0);
}

/**
 * Controller interaction
 */
void LocalPlannerManagerNode::control_send_goal(
    const nav_msgs::msg::Path::SharedPtr path, float target_spd) {

    RCLCPP_DEBUG(get_logger(), "sending goal to controller at [%s]",
                 this->controllerServerRoute.c_str());

    using namespace std::placeholders;

    if (!this->control_action_client_->wait_for_action_server()) {
        RCLCPP_ERROR(this->get_logger(), "Control action server not available, "
    ...
    auto send_goal_options =
        rclcpp_action::Client<ControlAction>::SendGoalOptions();
    send_goal_options.goal_response_callback = std::bind(
        &LocalPlannerManagerNode::control_action_goal_response_callback, this,
        _1);
    send_goal_options.feedback_callback =
        std::bind(&LocalPlannerManagerNode::control_action_feedback_callback,
                  this, _1, _2);
    send_goal_options.result_callback = std::bind(
        &LocalPlannerManagerNode::control_action_result_callback, this, _1);
...
void LocalPlannerManagerNode::control_action_feedback_callback(
    ...
    RCLCPP_DEBUG(get_logger(), "Currently at waypoint index: [{%d}]",
                 feedback->curr_index);
}
void LocalPlannerManagerNode::control_action_result_callback(
    ...
    case rclcpp_action::ResultCode::SUCCEEDED:
        RCLCPP_DEBUG(this->get_logger(),
                     "control_action result success received");
        break;
    case rclcpp_action::ResultCode::ABORTED:
...
void LocalPlannerManagerNode::p_PrintCostMapInfo(
    ...
    RCLCPP_INFO(this->get_logger(), "  resolution: %f",
